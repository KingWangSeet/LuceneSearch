...
//onblur事件
function useronblur(){//当第一个下拉框的选项发生改变时调用该函数
	var ptn = document.getElementById("userNo").value; //获取第一级菜单的值
	sendRequest("../user_uniqueness?userNo=" + ptn); //发送参数到SERVLET
...
function progessRequest() {
	if (XMLHttpReq.readyState == 4) {
		if (XMLHttpReq.status == 200) {
			...
			var s = xmlDoc.getElementsByTagName("user");//获取XML节点
			var obj = document.getElementById("userNoError");
			...
			if (xValue == "true") {//如果不为空就置空				
				...
				if(document.getElementById("userNo").value.length == 0){
					obj.innerHTML="请输入用户编号";
					obj.style.color="red";
				} else{
					obj.innerHTML="该用户编号可用";
